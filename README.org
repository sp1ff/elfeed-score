...
#+BEGIN_EXAMPLE
cd /tmp
curl -L --output=elfeed-score-0.4.1.tar.gz https://github.com/sp1ff/elfeed-score/archive/0.4.1.tar.gz
tar xvf elfeed-score-0.4.1.tar.gz && cd elfeed-score-0.4.1
./configure
make
make install
#+END_EXAMPLE
...
curl -L --output=elfeed-score-0.4.1.tar.gz https://github.com/sp1ff/elfeed-score/archive/0.4.1.tar.gz
tar xvf elfeed-score-0.4.1.tar.gz && cd elfeed-score-0.4.1
export EMACSLOADPATH=/tmp/elfeed-score-0.4.1:/tmp/elfeed/tests:$EMACSLOADPATH
./configure
make
make check
#+END_EXAMPLE
...
When I began this project, the only example of scoring for Elfeed I could find was [[https://kitchingroup.cheme.cmu.edu/blog/2017/01/05/Scoring-elfeed-articles/][this]] article ("Scoring Elfeed Articles"). The author (John Kitchin) computes a score and adds one or two tags to entries whose score is sufficiently high. It's always bothered me that elfeed-score couldn't do that, so in build 0.4.1, I added one more type of rule: "adjust-tags". These are applied _after_ the scoring process, and will add or remove tags based on whether the entry's score is above or below a given threshold.
...
I'm using =elfeed-score= day in & day out for my RSS reading, but this is a preliminary release (the version number, 0.4.1, was chosen to suggest this).
