...
#+BEGIN_EXAMPLE
cd /tmp
curl -L --output=elfeed-score-0.5.2.tar.gz https://github.com/sp1ff/elfeed-score/archive/0.5.2.tar.gz
tar xvf elfeed-score-0.5.2.tar.gz && cd elfeed-score-0.5.2
./configure
make
sudo make install
#+END_EXAMPLE
...
curl -L --output=elfeed-score-0.5.2.tar.gz https://github.com/sp1ff/elfeed-score/archive/0.5.2.tar.gz
tar xvf elfeed-score-0.5.2.tar.gz && cd elfeed-score-0.5.2
export EMACSLOADPATH=/tmp/elfeed-score-0.5.2:/tmp/elfeed/tests:$EMACSLOADPATH
./configure
make
make check
#+END_EXAMPLE
