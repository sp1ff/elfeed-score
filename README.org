...
#+BEGIN_EXAMPLE
cd /tmp
curl -L --output=elfeed-score-0.5.4.tar.gz https://github.com/sp1ff/elfeed-score/archive/0.5.4.tar.gz
tar xvf elfeed-score-0.5.4.tar.gz && cd elfeed-score-0.5.4
./configure
make
sudo make install
#+END_EXAMPLE
...
curl -L --output=elfeed-score-0.5.4.tar.gz https://github.com/sp1ff/elfeed-score/archive/0.5.4.tar.gz
tar xvf elfeed-score-0.5.4.tar.gz && cd elfeed-score-0.5.4
export EMACSLOADPATH=/tmp/elfeed-score-0.5.4:/tmp/elfeed/tests:$EMACSLOADPATH
./configure
make
make check
#+END_EXAMPLE
...
Many types of rules can be customized to only be applied to an entry if is (or is not) part of one or more feeds. Limiting rules by entry feeds involves the use of a cons cell of the form:
